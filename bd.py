...
class Programs_course(DeclBase):
    ...
    __table_args__ = {'extend_existing': True}
    __table_args__ = (PrimaryKeyConstraint('semester', 'course_id', 'program_id'),)
   
    semester = Column(Integer)
    course_id = Column(Integer, ForeignKey('courses.id'))
    program_id = Column(Integer, ForeignKey('programs.id'))
    
    def __init__(self, semester, course, program):
...
for p in session.query(Program):
    ...
    for pc in p.program_courses:
        print("   - ", pc.program.name, pc.course.name, pc.semester)
session.commit()
